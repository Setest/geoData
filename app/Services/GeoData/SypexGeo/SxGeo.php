...
class SxGeo
{
     ...
     * __construct.
     *
     * @param string $db_file Default: 'SxGeoCityMax.dat'
     * @param mixed  $type    Default: SXGEO_FILE
     * @return void
    ...
    public function __construct(?string $db_file = 'SxGeoCityMax.dat', int $type = SXGEO_FILE)
    {
        ...
        if (substr($header, 0, 3) != 'SxG') {
            throw new GeoDataException("Can't open $db_file", 1);
        }
        ...
        if ($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) {
            throw new GeoDataException("Wrong file format $db_file", 1);
        }
    ...
    {
        if (!$this->pack) {
            throw new GeoDataException("Can`t parse DB data", 1);
            // return false;
        }
